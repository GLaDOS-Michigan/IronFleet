namespace IronfleetTestDriver
{
    ...
    class Program
    {
        ...
        static void usage()
        {
            Console.WriteLine("Expected usage: clientIP IP0 port0 IP1 port1 IP2 port2 num_threads duration_secs workload (g|s) num_keys size_value_bytes 0");
        }
        ...
        {            
            ...
            try
            {
                ...
                num_threads = Convert.ToUInt64(args[3]);
                experiment_duration = Convert.ToUInt64(args[4]);

                workload = Convert.ToChar(args[5]);
                num_keys = Convert.ToUInt64(args[6]);
                size_value = Convert.ToUInt64(args[7]);
                clientId_startIdx = 0; // Convert.ToUInt64(args[8]);
            }
            catch (Exception e)
            {
            ...
            // Setup the system
            ulong num_setup_threads = 1;
            var setup_threads = ClientBase.StartSetupThreads<IronSHT.Client>(num_setup_threads, num_keys, size_value).ToArray();
            setup_threads[0].Join();
            
            // Start the experiment
            var threads = ClientBase.StartExperimentThreads<IronSHT.Client>(num_threads, num_keys, size_value, workload, clientId_startIdx).ToArray();
