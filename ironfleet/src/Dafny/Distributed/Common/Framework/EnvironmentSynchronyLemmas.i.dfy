...
{
    reveal_QueuePosition();
}

lemma Lemma_QueuePositionTail<IdType, MessageType>(q:seq<LPacket<IdType, MessageType>>, p:LPacket<IdType, MessageType>, q':seq<LPacket<IdType, MessageType>>)
    requires p in q;
        ...
        ensures sat(synchrony_start, eventual(HostQueueEmptyTemporal(b, host)));
    {
            ...
            ensures sat(i1, nextOrDecrease(goal, d));
        {
            if (!sat(i1, eventual(or(goal, nextDecrease(i1, d)))))
            {
                ...
                forall ensures sat(i1, countWithinLe(bs * p, incr, (p + 1) * p, timefun));
                {
                    assert sat(synchrony_start, always(NetworkDeliveryRateForHostBoundedTemporal(b, burst_size, burst_size * receive_period + 1, host)));
                    assert sat(i1, always(NetworkDeliveryRateForHostBoundedTemporal(b, burst_size, burst_size * receive_period + 1, host)));
                    assert sat(i1, always(countWithinLe(bs, incr, p + 1, timefun)));
                    Lemma_DeliverIsInstantaneous(b, i1, host);
    ...
    ensures  processing_bound == latency_bound + burst_size * receive_period;
{
        ...
        ensures sat(i1, AllPacketsReceivedWithinTemporal(b, latency_bound + burst_size * receive_period, sources, destinations));
    {
            ...
            ensures sat(i1, next(eventuallynextwithin(PacketReceivedTemporal(b, p), latency_bound + burst_size * receive_period, BehaviorToTimeMap(b))));
        {
            ...
            {
                TemporalAssist();
                assert sat(i1+1, eventuallynextwithin(PacketDeliveredTemporal(b, p), latency_bound, BehaviorToTimeMap(b)));
            }
    ...
    ensures  sat(processing_sync_start, always(AllPacketsReceivedWithinTemporal(b, latency_bound + burst_size * receive_period, sources, destinations)));
{
    ...
    {
        Lemma_AlwaysImpliesLaterAlways(synchrony_start, i1, eventuallynextwithin(ReceiveAttemptedTemporal(b, host), receive_period, BehaviorToTimeMap(b)));
        Lemma_AlwaysImpliesLaterAlways(synchrony_start, i1, NetworkDeliveryRateForHostBoundedTemporal(b, burst_size, burst_size * receive_period + 1, host));
    }
    processing_bound := Lemma_EventuallyAllPacketsAlwaysReceivedInTimeHelper3(i1, latency_bound, sources, destinations, b, burst_size, receive_period);
    ...
    ensures  sat(processing_sync_start, always(AllPacketsReceivedWithinTemporal(b, latency_bound + burst_size * receive_period, sources, destinations)));
{
    ...
    processing_sync_start, processing_bound := Lemma_EventuallyAllPacketsAlwaysReceivedInTimeHelper2(synchrony_start, i1, latency_bound, sources, destinations, b, burst_size, receive_period);
}
