...
//////////////////
    ...
    lemma Lemma_LeadsToTwoPossibilitiesWithinWithFirstStepAnAction(i:int, w:temporal, x:temporal, y:temporal, z:temporal, t1:int, t2:int, timefun:imap<int, int>)
        requires imaptotal(timefun);
        ...
        ensures  sat(i, leadstonextwithin(x, and(not(y), next(y)), t, timefun));
    {
            ...
            ensures sat(j, imply(x, eventuallynextwithin(and(not(y), next(y)), t, timefun)));
        {
            if sat(j, x)
            {
                ...
                assert sat(m-1, and(not(y), next(y)));
            }
        }
    }
