    ...
    decreases (|schedule| + action_type - next_action_type) % |schedule|;
{
    ...
    {
        action_step := Lemma_RoundRobinSchedulerEventuallyPerformsNextAction(behavior, next_action_type_fun, scheduler_action, schedule, start_step, earliest_step, action_type);
        return;
    }

    var earlier_action_step := Lemma_RoundRobinSchedulerEventuallyPerformsNextAction(behavior, next_action_type_fun, scheduler_action, schedule, start_step, earliest_step, next_action_type);

    lemma_mod_auto(|schedule|);

    action_step := Lemma_RoundRobinSchedulerEventuallyPerformsCertainNextAction(behavior, next_action_type_fun, scheduler_action, schedule, start_step, earlier_action_step + 1, (next_action_type + 1) % |schedule|, action_type);
}
    ...
    ensures  sat(action_step, schedule[action_type]);
{
    ...
    action_step := Lemma_RoundRobinSchedulerEventuallyPerformsCertainNextAction(behavior, next_action_type_fun, scheduler_action, schedule, start_step, earliest_step, next_action_type, action_type);
}
