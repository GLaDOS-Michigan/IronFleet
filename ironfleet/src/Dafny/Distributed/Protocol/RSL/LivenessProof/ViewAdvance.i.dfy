...
lemma lemma_OneLiveReplicaSoonAdvancesSomeForever(
    ...
    ensures  0 <= ahead_idx < |b[start_step].replicas|;
    ensures  start_step <= step;
    ensures  forall moved_idx :: moved_idx in moved_indices ==> sat(step, always(HostReachedViewTemporal(b, moved_idx, CurrentViewOfHost(b[start_step], ahead_idx))));
    ensures  b[step].environment.time <= b[start_step].environment.time + TimeForOneReplicaToAdvanceAnother(asp, processing_bound);
...
lemma lemma_OneLiveReplicaSoonAdvancesAllForever(
    ...
    ensures  0 <= ahead_idx < |b[start_step].replicas|;
    ensures  start_step <= step;
    ensures  forall moved_idx :: moved_idx in asp.live_quorum ==> sat(step, always(HostReachedViewTemporal(b, moved_idx, CurrentViewOfHost(b[start_step], ahead_idx))));
    ensures  b[step].environment.time <= b[start_step].environment.time + TimeForOneReplicaToAdvanceAnother(asp, processing_bound);
{
