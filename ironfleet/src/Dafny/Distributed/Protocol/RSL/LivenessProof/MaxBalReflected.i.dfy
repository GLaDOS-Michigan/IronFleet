...
{
    (forall idx{:trigger PrimaryHasReachedState2OfBallot(ps, ps.replicas[idx].replica.executor.max_bal_reflected)} :: 0 <= idx < |ps.replicas| ==> PrimaryHasReachedState2OfBallot(ps, ps.replicas[idx].replica.executor.max_bal_reflected))
    && (forall p{:trigger p.msg.RslMessage_AppStateSupply?} :: p in ps.environment.sentPackets && p.src in ps.constants.config.replica_ids && p.msg.RslMessage_AppStateSupply? ==> PrimaryHasReachedState2OfBallot(ps, p.msg.bal_state_supply))
}
