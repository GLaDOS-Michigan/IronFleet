    ...
    requires CBallotIsAbstractable(ballot);
{
    ...
    Ballot(int(ballot.seqno), int(ballot.proposer_id))
}
...
{
    Request(AbstractifyEndPointToNodeIdentity(c.client), int(c.seqno), AbstractifyCAppMessageToAppMessage(c.request))  
}
    ...
    ensures forall o :: o in newDomain ==> COperationNumber(uint64(o)) in m;
{
    forall o | o in newDomain
        ...
        ensures COperationNumber(uint64(o)) in m;
    {
        var i :| i in m && o==AbstractifyCOperationNumberToOperationNumber(i);
        assert 0<=int(i.n)<0x10000000000000000;
    }
}
