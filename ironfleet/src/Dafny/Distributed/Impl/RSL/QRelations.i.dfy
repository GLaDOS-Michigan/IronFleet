...
{
    reveal_Q_LReplica_Next_Process_Invalid();
    ...
    reveal_Q_LReplica_Next_ProcessPacketWithoutReadingClock();
}
...
lemma lemma_EstablishQLReplicaNoReceiveNextFromNoClock(replica:LReplica, replica':LReplica, sent_packets:seq<RslPacket>, nextActionIndex:int, ios:seq<RslIo>)
    ...
    ensures Q_LReplica_NoReceive_Next(replica, int(nextActionIndex), replica', ios);
{
    reveal_Q_LReplica_Next_Spontaneous_MaybeEnterNewViewAndSend1a();
    reveal_Q_LReplica_Next_Spontaneous_MaybeEnterPhase2();
    //reveal_Q_LReplica_Next_Spontaneous_MaybeNominateValueAndSend2a();
    reveal_Q_LReplica_Next_Spontaneous_TruncateLogBasedOnCheckpoints();
    reveal_Q_LReplica_Next_Spontaneous_MaybeMakeDecision();
    reveal_Q_LReplica_Next_Spontaneous_MaybeExecute();

    assert LReplicaNoReceiveNext(replica, int(nextActionIndex), replica', ios);
    reveal_Q_LReplica_NoReceive_Next();
    assert Q_LReplica_NoReceive_Next(replica, int(nextActionIndex), replica', ios);
}
