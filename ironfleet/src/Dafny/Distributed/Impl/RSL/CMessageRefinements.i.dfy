    ...
    requires CMessageIsAbstractable(msg);
{
    match msg
        ...
        case CMessage_Request(seqno, value) => RslMessage_Request(int(seqno), AbstractifyCAppMessageToAppMessage(value))
        ...
        case CMessage_Reply(seqno, reply) => RslMessage_Reply(int(seqno), AbstractifyCAppMessageToAppMessage(reply))
...
lemma lemma_AbstractifyCPacketToRslPacket_isInjective()
    ensures forall p1, p2 ::
           CPacketIsInjectiveType(p1) && CPacketIsInjectiveType(p2)
        && AbstractifyCPacketToRslPacket(p1) == AbstractifyCPacketToRslPacket(p2)
        ==> p1 == p2
{
    forall p1, p2 | CPacketIsInjectiveType(p1) && CPacketIsInjectiveType(p2)
        && AbstractifyCPacketToRslPacket(p1) == AbstractifyCPacketToRslPacket(p2)
        ensures p1 == p2;
    {
