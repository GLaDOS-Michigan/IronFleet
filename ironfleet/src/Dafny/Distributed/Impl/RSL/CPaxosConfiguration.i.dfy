...
datatype CPaxosConfiguration = CPaxosConfiguration(
...
{
    0 <= int(index) < |config.replica_ids|
}
...
{
    if (CPaxosConfigurationIsAbstractable(config)
        && 0 < |config.replica_ids|)
    {
...
method CGetReplicaIndex(replica:EndPoint, config:CPaxosConfiguration) returns (found:bool, index:uint64)
    ...
    ensures  found ==> GetReplicaIndex(AbstractifyEndPointToNodeIdentity(replica), AbstractifyCPaxosConfigurationToConfiguration(config)) == int(index);
    ...
    ensures !found ==> !(AbstractifyEndPointToNodeIdentity(replica) in AbstractifyEndPointsToNodeIdentities(config.replica_ids));
{
    ...
    while i < uint64(|config.replica_ids|)
        invariant i < uint64(|config.replica_ids|);
        invariant forall j :: 0 <= j < i ==> config.replica_ids[j] != replica;
    {
        if replica == config.replica_ids[i] {
            ...
            calc ==> {
                true;
                    { reveal_SeqIsUnique(); }
                forall j :: 0 <= j < |config.replica_ids| && j != int(i) ==> config.replica_ids[j] != replica;
            }
        ...
        if i == uint64(|config.replica_ids|) - 1 {
            found := false;
            return;
        }
