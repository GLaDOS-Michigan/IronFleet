...
{
    var batchArr := new CRequest[uint64(|val.a|)];

    var i:uint64 := 0;
    while i < uint64(|val.a|) 
        invariant 0 <= int(i) <= |val.a|;
        invariant forall j :: 0 <= j < int(i) ==> batchArr[j] == parse_Request(val.a[j]);
    {
        var req := parse_Request(val.a[i]);
        batchArr[i] := req;
        i := i + 1;
    ...
    ensures  CBallotIsAbstractable(parse_Ballot(val));
{
    ...
    CBallot(val.t[uint64(0)].u, val.t[uint64(1)].u)
}
...
{
    CVote(parse_Ballot(val.t[uint64(0)]), parse_RequestBatch(val.t[uint64(1)]))
}
...
{
    var batch := Parse_RequestBatch(val.t[uint64(1)]);
    vote := CVote(parse_Ballot(val.t[uint64(0)]), batch);
}
    ...
    decreases |val.a|;
{
    if |val.a| == 0 then
        ...
        var op := parse_OperationNumber(tuple.t[uint64(0)]);
        var vote := parse_Vote(tuple.t[uint64(1)]);
        var others := parse_Votes(VArray(val.a[uint64(1)..]));
        calc {
            ValidVal(val);
    ...
    ensures  ValidVal(val) ==> CMessageIs64Bit(parse_Message_Request(val));
{
    ...
    CMessage_Request(val.t[uint64(0)].u, parse_AppMessage(val.t[uint64(1)]))
}
...
{
    CMessage_1b(parse_Ballot(val.t[uint64(0)]), parse_OperationNumber(val.t[uint64(1)]), parse_Votes(val.t[uint64(2)]))
}
...
{
    var votes := Parse_Votes(val.t[uint64(2)]);
    msg := CMessage_1b(parse_Ballot(val.t[uint64(0)]), parse_OperationNumber(val.t[uint64(1)]), votes);
}
...
{
    CMessage_2a(parse_Ballot(val.t[uint64(0)]), parse_OperationNumber(val.t[uint64(1)]), parse_RequestBatch(val.t[uint64(2)]))
}
...
method Parse_Message_2a(val:V) returns (msg:CMessage)
    ...
    ensures  CMessageIs64Bit(msg);
{
    var batch := Parse_RequestBatch(val.t[uint64(2)]);
    msg := CMessage_2a(parse_Ballot(val.t[uint64(0)]), parse_OperationNumber(val.t[uint64(1)]), batch);
}
...
{
    CMessage_2b(parse_Ballot(val.t[uint64(0)]), parse_OperationNumber(val.t[uint64(1)]), parse_RequestBatch(val.t[uint64(2)]))
}
...
{
    var batch := Parse_RequestBatch(val.t[uint64(2)]);
    msg := CMessage_2b(parse_Ballot(val.t[uint64(0)]), parse_OperationNumber(val.t[uint64(1)]), batch);
}
    ...
    ensures  ValidVal(val) ==> CMessageIs64Bit(parse_Message_Heartbeat(val));
{
    ...
    CMessage_Heartbeat(parse_Ballot(val.t[uint64(0)]), val.t[uint64(1)].u != 0, parse_OperationNumber(val.t[uint64(2)]))
}
...
{
    CMessage_AppStateRequest(parse_Ballot(val.t[uint64(0)]), parse_OperationNumber(val.t[uint64(1)]))
}
...
{
    CMessage_AppStateSupply(parse_Ballot(val.t[uint64(0)]), parse_OperationNumber(val.t[uint64(1)]), parse_AppState(val.t[uint64(2)]), parse_ReplyCache(val.t[uint64(3)]))
}
...
{
    CMessage_StartingPhase2(parse_Ballot(val.t[uint64(0)]), parse_OperationNumber(val.t[uint64(1)]))
}
...
{
       (!msg.CMessage_Invalid?)
    && (msg.CMessage_Request? ==> ValidAppMessage(msg.val))
    ...
    && (msg.CMessage_Reply? ==> ValidAppMessage(msg.reply))
    && (msg.CMessage_AppStateRequest? ==> true)
    && (msg.CMessage_AppStateSupply? ==> AppStateMarshallable(msg.app_state) && ValidReplyCache(msg.reply_cache))
...
{
    b := uint64(|vote.max_val|) <= 100;  // RequestBatchSizeLimit
}
...
{
    b := uint64(|votes.v|) < 8;  // max_votes_len
    if !b {
        return;
    }
...
{
    b := uint64(|m|) < 256; // RequestBatchSizeLimit

    assert CReplyCacheIsAbstractable(m);
    forall e | e in m
        ensures ValidReply(m[e]);
    {
    ...
    ensures  b == Marshallable(msg);
{
    if msg.CMessage_Invalid? {
        ...
        b := uint64(|msg.val_2a|) <= 100; // RequestBatchSizeLimit()
    }
    else if msg.CMessage_2b? {
        b := uint64(|msg.val_2b|) <= 100; // RequestBatchSizeLimit()
    }
    else if msg.CMessage_Heartbeat? {
        b := true;
    }
    else if msg.CMessage_Reply? {
        b := ValidAppMessage(msg.reply);
    }
    ...
    ensures BufferRefinementAgreesWithMessageRefinement(msg, data[..]);
{
    ...
    forall src, dst | EndPointIsValidIPV4(src) && EndPointIsValidIPV4(dst) 
        ensures AbstractifyBufferToRslPacket(src, 
                                     dst, 
                                     data[..])
                == LPacket(AbstractifyEndPointToNodeIdentity(dst), AbstractifyEndPointToNodeIdentity(src), AbstractifyCMessageToRslMessage(msg));
    {
        calc {
            AbstractifyBufferToRslPacket(src, 
                                 dst, 
                                 data[..]);
            LPacket(AbstractifyEndPointToNodeIdentity(dst),
                   AbstractifyEndPointToNodeIdentity(src),
